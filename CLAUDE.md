...
Public hosting remains fully out of scope — considered and explicitly declined (Cloudflare Tunnel + Access was evaluated and its repo-side scaffolding briefly existed, then reverted at the user's request in favor of `tailscale serve`, which stays tailnet-only and doesn't touch this boundary at all; see README's "Optional: HTTPS within your tailnet via `tailscale serve`").
