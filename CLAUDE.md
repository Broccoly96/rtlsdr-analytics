...
Three deliberate exceptions to "no real-time tracking" exist. Two are narrow, scoped to one explicitly-selected aircraft: `WS /ws/aircraft/{icao}` (`app/api/routers/aircraft_live.py`) polls readsb independently, at the same cadence as the collector, and streams tar1090-parity fields this app doesn't store (squawk, NAC/SIL/NIC, FMS-selected altitude/heading, wind, mach, ...) to the aircraft-detail sidebar (`app/static/js/aircraftinfo.js`) only. The third is broader: the 3D flight globe's (`app/static/globe.html`) *default* view genuinely is a live map of every currently-received aircraft's position/callsign/altitude/track/speed/roll/vertical-rate, rendered as a 3D aircraft model oriented by heading/roll/approximated-pitch (not the full tar1090 field set, and confined to this one page, not the primary dashboard), fed by a single shared `WS /ws/aircraft-positions` broadcast (`app/api/routers/aircraft_positions.py`) — one server-side readsb poll fanned out to every connected client rather than one poll per aircraft, at `POLL_INTERVAL_SECONDS` by default or 1s (`FAST_POLL_INTERVAL_SECONDS`) if any connected client has opted in via `{"fast": true}` over the same socket, reverting once none do. This same broadcast also backs the globe's shift+click "isolate one aircraft" mode; no separate per-aircraft WS is opened for that. The globe's header can instead switch to a non-live "history" mode (`GET /api/tracks?hours=`, the same endpoint the flat 2D map uses, now emitting 3D `[lon, lat, altitude_ft]` coordinates instead of 2D-only) showing every aircraft's past track with no live broadcast/3D models involved at all. None of the three real-time exceptions persist anything.
