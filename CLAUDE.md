...
Two deliberate, narrow exceptions to "no real-time tracking" exist, both scoped to one explicitly-selected aircraft rather than a live map of everything: `WS /ws/aircraft/{icao}` (`app/api/routers/aircraft_live.py`) polls readsb independently, at the same cadence as the collector, and streams tar1090-parity fields this app doesn't store (squawk, NAC/SIL/NIC, FMS-selected altitude/heading, wind, mach, ...) to the aircraft-detail sidebar (`app/static/js/aircraftinfo.js`) and the 3D flight globe (`app/static/globe.html`). Neither persists anything.
