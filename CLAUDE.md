...
Three deliberate exceptions to "no real-time tracking" exist. Two are narrow, scoped to one explicitly-selected aircraft: `WS /ws/aircraft/{icao}` (`app/api/routers/aircraft_live.py`) polls readsb independently, at the same cadence as the collector, and streams tar1090-parity fields this app doesn't store (squawk, NAC/SIL/NIC, FMS-selected altitude/heading, wind, mach, ...) to the aircraft-detail sidebar (`app/static/js/aircraftinfo.js`) and, when an aircraft is isolated there, the 3D flight globe (`app/static/globe.html`). The third is broader: the 3D flight globe's *default* view genuinely is a live map of every currently-received aircraft's position/callsign/altitude/track/speed (not the full tar1090 field set, and confined to this one page, not the primary dashboard), fed by a single shared `WS /ws/aircraft-positions` broadcast (`app/api/routers/aircraft_positions.py`) — one server-side readsb poll fanned out to every connected client rather than one poll per aircraft. None of these three persist anything.
